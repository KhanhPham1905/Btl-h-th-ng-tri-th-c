...
# 5. câu hỏi về vị trí chấn thương đã từng bị chấn thương hay không
def first4_question(list_symptom_of_person, person):
    global ever_suffered
    while (1):
        check = True
        benh =''
        listBenh=['', 'chấn thương xương', 'căng cơ bắp', 'trật khớp', 'bong gân', 'viêm gân', 'đứt dây chằng']
        if (check):
            print(f'-->Chatbot: {person.name} có thể cho mình biết vị trí bạn bị trấn thương đã từng bị trấn thương nào trong các chấn thương sau đây  chưa:')
            print('1. chấn thương xương')
            print('2. căng cơ bắp')
            print('3. trật khớp')
            print('4. bong gân')
            print('5. viêm gân')
            print('6. đứt dây chằng')
            print('0. chưa từng bị')
            print('---------------Câu trả lời của bạn---------------')
            answer = validate.validate_input_number_form(input())
            # print("Người dùng: Lựa chọn của tôi ", answer)
            print(f'-->{person.name}: Lựa chọn của tôi {answer}')
            if (int(answer) < 0 or int(answer) > 6):
                ...
                ever_suffered = 'D'+answer
                benh=listBenh[int(answer)]
                break
        else:
            break

    print(f'-->Chatbot: Chúng tôi đã ghi nhận chấn thương {person.name} đã từng bị là:',
          benh)
    return list_symptom_of_person
...
list_symptom_of_person = first2_question(list_symptom_of_person, person)
list_symptom_of_person = first3_question(list_symptom_of_person, person)
list_symptom_of_person = first4_question(list_symptom_of_person, person)
